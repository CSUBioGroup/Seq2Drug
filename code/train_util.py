...
class TrainLoop:
    ...
    def forward_only(self, batch):
        with th.no_grad():
            ...
            for i in range(0, len(batch['SMILES']), self.microbatch):
                smiles = batch['SMILES'][i : i + self.microbatch]
                sequences = batch['target_sequence'][i : i + self.microbatch]

                mol_batch = self.mol_model.batch_encode(smiles)
                mol_encodes = mol_batch["memory_input"].transpose(0, 1)
                prot_encodes = self.get_prot_inputs(sequences)

                mol_inputs = self.model.mol_layer(mol_encodes)
                prot_inputs = self.model.prot_layer(prot_encodes)

                micro = th.cat((prot_inputs, mol_inputs), dim=1)

                mask = th.full([micro.shape[0], micro.shape[1]], 0, dtype=th.int64)
                mask[:, prot_inputs.shape[1]:] = 1
                ...
                last_batch = (i + self.microbatch) >= len(batch['SMILES'])
    ...
    def get_prot_inputs(self, sequences):
        demo_repr = self.prot_feats[sequences[0]]
        maxn = max([self.prot_feats[seq].shape[1] for seq in sequences])
        sequence_repr = th.zeros([len(sequences), maxn, demo_repr.shape[-1]], dtype=demo_repr.dtype)
        for i, seq in enumerate(sequences):
            repr = self.prot_feats[seq]
    ...
    def save(self):
        ...
        if dist.get_rank() == 0:
            mol_filename = f"mol_model_{(self.step+self.resume_step):06d}.pth"
            print('writing to', bf.join(self.checkpoint_path, mol_filename))
            mol_path = bf.join(self.checkpoint_path, mol_filename)
            th.save(self.mol_model, mol_path)
