...
if __name__ == '__main__':
    ...
    parser.add_argument('--mol_pattern', type=str, default='mol_model', help='training pattern')
    ...
    for lst in glob.glob(args.model_dir):
        ...
        molbart_checkpoints = sorted(glob.glob(f"{lst}/{args.mol_pattern}*.pth"))[::-1]

        assert len(checkpoints) == len(molbart_checkpoints)
        ...
        for i in range(len(checkpoints)):
            ...
            checkpoint_one = checkpoints[i]
            molbart_one = molbart_checkpoints[i]

            COMMAND = f'python -m torch.distributed.launch --nproc_per_node=1 --master_port={12233 + int(args.seed)} --use_env sample_seq2mol.py ' \
            f'--model_path {checkpoint_one} --molbart_checkpoint {molbart_one} --step {args.step} ' \
