...
def sdf_to_pdbqt(sdf_file, pdbqt_outfile, mol_id):
    ...
    os.popen(f'obabel {sdf_file} -O {pdbqt_outfile} ').read()
    return pdbqt_outfile


def calculate_qvina2_score(receptor_file, sdf_file, out_dir, size=20, exhaustiveness=16, index=None, r_mol=None):
    ...
    for i, mol in enumerate(suppl):  # sdf file may contain several ligands
        ...
        if r_mol is not None:
            cx, cy, cz = r_mol.GetConformer().GetPositions().mean(0)
        else:
