...
def calculate_diversity(pocket_mols):
    ...
    # fps = [AllChem.GetMorganFingerprintAsBitVect(mol, 2, nBits=1024) for mol in pocket_mols]
    div = 0
    total = 0
    for i in tqdm(range(len(pocket_mols))):
        for j in range(i + 1, len(pocket_mols)):
            div += 1 - DataStructs.TanimotoSimilarity(fps[i], fps[j])
            total += 1
    return div / total
