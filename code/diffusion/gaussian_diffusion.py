...
class GaussianDiffusion:
    ...
    def training_losses(self, model, mol_model, mol_batch, *args, **kwargs):
        self.model = model
        return self.training_losses_seq2seq(model, mol_model, mol_batch, *args, **kwargs)
    ...
    def training_losses_seq2seq(self, model, mol_model, mol_batch, x_start, t, model_kwargs=None, noise=None):
        """
        ...
        input_mask = model_kwargs.pop('input_mask').to(t.device)
        fine_ture = model_kwargs.pop('fine_tune')
        ...
        x_t = self.q_sample(x_start, t, noise=noise, mask=input_mask) # reparametrization trick.
        ...
        # terms["mse"] = th.where(t0_mask, t0_loss, mse)

        # out_mean, _, _ = self.q_mean_variance(x_start, th.LongTensor([self.num_timesteps - 1]).to(x_start.device))
        # tT_loss =  mean_flat(out_mean ** 2)
        # terms["tT_loss"] = tT_loss

        unmask_len = int(sum(input_mask[0] == 0))
        mol_diff = model_out_x_start[:, unmask_len:, :]
        mol_diff_mem = model.model.module.mol_out_layer(mol_diff).transpose(0, 1)
        mol_batch["memory_input_"] = mol_diff_mem

        terms["token_mse"] = mean_flat((mol_batch["memory_input"].transpose(0, 1) - mol_batch["memory_input_"].transpose(0, 1)) ** 2)

        # terms["loss"] = terms["mse"] + terms["tT_loss"] + terms["token_mse"]
        # terms["loss"] = terms["mse"] + terms["tT_loss"]
        terms["loss"] = terms["token_mse"]

        if fine_ture:
            mol_output = mol_model.batch_decode(mol_batch)
            terms["decoder_nll"] = mol_model._calc_loss(mol_batch, mol_output)
            terms["loss"] += terms["decoder_nll"]
            # terms["loss"] = terms["decoder_nll"]
...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def training_losses(
        self, model, mol_model, mol_batch, *args, **kwargs
    ):  # pylint: disable=signature-differs
        # print('called training_losses')
        return super().training_losses(self._wrap_model(model), mol_model, mol_batch, *args, **kwargs)
